...
total_frames = 0
first_frame = None
block_duration = 10  # Duration of each block in seconds (5 minutes)
...
# Process each frame from the recorded video
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
    ...
    # Process frames only every 5 minutes
    if total_frames % (block_duration * fps) == 0:
        # Add the frame to the current block
        frames_in_block.append(frame)

    if current_time % block_duration == 0 and current_time != 0 and frames_in_block:
        # If we have completed a block of 5 minutes, analyze the frames
